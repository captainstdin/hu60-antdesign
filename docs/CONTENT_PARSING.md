...
> 唯一的例外是 AI 助手的结果展示：AI 回复不经过论坛后端，无法走 `parseContent`，因此 `src/utils/markdown.js` 实现了一个只覆盖常见语法的轻量渲染器（代码块、标题、列表、表格、引用、行内格式），输出后同样交给 `sanitizeHtml` 清洗。它只服务于 AI 结果面板，**不参与帖子正文的解析**，也不改变上面「帖子正文走后端」的约定。
